...
This documentation is valid for Onezone versions from `18.02.0-rc12` to `18.02.*`. 
...
The `auth.config` file has been reworked completely. During the first startup
after upgrading to `18.02.0-rc12` or newer, Onezone will perform 
...
`onepanelAuthConfig` - this section enables or disables onepanelAuth - the 
...
REST API operations using basic credentials.

> Note that to allow for login via Onezone GUI, `onepanelAuth` must be enabled,
and present on the list of supported IdPs ([see below](#supported-idps)).

> Onepanel admin is always created during installation process. Additional users
can be created [using the API](../creating_user_accounts.md).
    ...
    openidConfig => #{
        ...
        defaultProtocolConfig => #{
            ...
            attributeMapping => #{
                ...
                name => {required, {any, ["name", "fullName"]}},
...
```Erlang
    supportedIdps => [
        ...
        {myIdP, #{
            ...
            %   /var/www/html/custom/oz_worker/<path>
...
for more). 
> Note that the identifier *onepanel* is reserved for 
[onepanelAuth](#onepanelauth-example) and *more* is reserved for 
...
> Note that disabling any protocol in `onepanelAuthConfig`, `openidConfig` or
...
* `subjectId` - user's unique identifier within the IdP, must be persistent
(unchanging in time), mandatory (login will fail if mapping cannot be found)

* `name` - user's displayed name

* `alias` - user's short, unique identifier - used as an alternative to 
identifying users by name, is optional and can be set/unset anytime
...
that he logged in with, according to the following formula:
```
...
`subjectId` in the first IdP the user has logged in with). `name` and `alias` 
will also be unchanged by consecutive log-ins, even if the IdP attributes 
change. `emails` are always a list of emails aggregated from all linked 
accounts.
...
```Erlang
 attributeMapping => #{
    subjectId => {required, "eduPersonTargetedID"},
    name => {optional, {any, ["displayName", {concat, ["givenName", {str, " "}, "surName"]}]}},
    alias => {optional, "eduPersonPrincipalName"},
...
path `/var/www/html/custom/oz_worker/<path>` and reference it in the config like 
...
> NOTE: The path changes to `/var/www/html/oz_worker/custom/<path>` in Onezone
versions `19.02.*`.
