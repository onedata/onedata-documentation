...
The process of creating an archive comes with several options:

* different layouts (plain and [BagIt][bagit-and-dip]) — the structure of the files in the
  created archive,
...
* [including DIP][bagit-and-dip] — adding a Dissemination Information Package view for the
  archive,
...
In case the archiving process **fails**, the state of the archive becomes:
...
You can click on the **Failed** badge and open an archiving [audit log][] with error
reason information.

![screen-archive-failed-row][]

### Cancelling archiving

The archiving process can be cancelled.
...
* **table column headers** — presenting information about archives:
  * **State** — what stage of lifecycle the archive is in and how much data it contains
...
* **Show audit log** — showing the archiving audit log tab in the [archive details
  panel][],
...
* **Browse DIP** — opening the [DIP][bagit-and-dip] (Dissemination Information Package)
  view of the archive files browser (if available),
* **Delete archive** — opening the modal for [deleting][delete] the archive.

### Archive details panel

The archive details panel is accessible from the
[context menu of archive][archive actions].
...
* **Properties** — showing the basic information about the archive, including the editable
  archive description and initial configuration set when the archive was
  [created][Create Archive],
...
entry shows granulated details about the process, such as archiving times, file
location, and its ID.

![screen-audit-log-details][]

In case of the archiving failure, you can also check the error reason.

![screen-audit-log-failed][]

::: tip NOTE
In rare cases, the data could be archived, but it has been not successfully validated
against its source — in this situation, there are entries both for successful data
archiving and verification error in the log for the file.
:::
...
* while the **Results** dataset's archives are created separately — each archive has a
  separate copy of the data, even unmodified.
...
2. Make sure that the contents of the dataset have been modified since the last taken
   snapshot, e.g. upload new files into the dataset. This step is necessary to see how the
   increment is handled later.
...
3. Open the context menu for an archive that should serve as a base for a new incremental
   ...
   the dataset, compared to the selected archive, will be hard-linked instead of being
   copied.
...
As the dataset nested structures are also important when archives are created, users may
choose to **create nested archives** on the level of ancestor dataset. This way, a set of
linked archives will be created.
...
archive created from the **Results** dataset directly.
...
2. Open the **Create archive** modal as described in the
   [creating archives][create archive] section.
...
Information Package of the archive, which presents the extra BagIt structure. Furthermore,
by **including the DIP**, you can also have a view of only the payload of the filesystem,
...
5. Click on the **Recall** button in the bottom-right corner of the panel.
...
While the recall is in progress, you can cancel the process using the **Cancel recall**
button in the bottom-left corner of the **Archive recall information** panel:
...
another archive. That means while deleting the monolithic or
[base archives][incremental archives] is allowed, you cannot delete the [nested
archives][] until it is not a top-most ancestor of the hierarchy.
:::
...
[bagit-and-dip]: #bagit-archives-and-dip
