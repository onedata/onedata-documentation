...
An inventory is a logical container used to store workflow and lambda schemas.
Similarly to Spaces, it allows users to collaborate on shared resources and
manage access rights.

## Workflow

A workflow definition describes what should be done. It is created using the graphical user interface (GUI) and
...
To learn about workflow execution lifecycle check: ...
...
![screen-lane-composition][screen-lane-composition]
...
![screen-lane-creation][screen-lane-creation]
...
Follow the detailed lambda example with step-by-step tutorial comments to learn
how to implement the lambda: [demo lambda][demo-lambda-handler].

Adding the lambda requires providing the following configuration in the GUI:

![screen-lambda-gui][screen-lambda-gui]

![screen-lambda-gui-resources][screen-lambda-gui-resources]
...
5. **Read-only**: indicate whether the lambda is allowed to modify data in a Space.
6. **Mount Space**: choose whether the lambda should use the Oneclient interface.
   ...
   [Oneclient documentation][oneclient-documentation]. Note that options 7 and 8
   become available only when **Mount Space** is enabled.
7. **Mount Point**: specify the directory path where Oneclient will be mounted.
   This path must match the expectations of the lambda implementation.
8. **Oneclient options**: configure additional Oneclient flags used during mounting.
9. **Preferred batch size**: define how many items the lambda should process
   in a single batch.
10. **Configuration parameters**: define configuration parameters that the lambda
...
11. **Arguments**: define the input arguments that the lambda logic receives
    when the task runs.
12. **Results**: define the results produced by the lambda.
13. **Resources**: specify requested values or limits for CPU cores, memory,
